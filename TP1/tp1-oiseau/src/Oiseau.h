...
const float LONG_MEMBRE = 1.0;
const float LARG_MEMBRE = 1.0;
...
public:
	...
	void afficherTheiere()
	{
		...
		// On effectue un rendu de la theiere à partir du tableau de connectivité 
	...
	void afficherAntenne()
	{
		...
		matrModel.PushMatrix(); {
			matrModel.Rotate(2 * angleTete, 0.0, 0.0, 1.0);
			matrModel.Translate(0.0, 0.0, 2 * taille); // bidon à modifier
	...
	void afficherAiles()
	{
		...
		matrModel.PushMatrix(); {
			...
			matrModel.Scale(2 * taille, 2 * taille, 0.0);
			// afficherRepereCourant( ); // débogage: montrer le repère à la position courante
			glUniformMatrix4fv(locmatrModel, 1, GL_FALSE, matrModel);
			afficherQuad();
		}matrModel.PopMatrix(); glUniformMatrix4fv(locmatrModel, 1, GL_FALSE, matrModel);
		...
		matrModel.PushMatrix(); {
			matrModel.Rotate(angleAile, -1.0, 0.0, 0.0);
			matrModel.Translate(-taille, -3 * taille, 0.0);
			matrModel.Scale(2 * taille, 2 * taille, 0.0);
			// afficherRepereCourant( ); // débogage: montrer le repère à la position courante
			glUniformMatrix4fv(locmatrModel, 1, GL_FALSE, matrModel);
			afficherQuad();
	...
	void afficherBras()
	{
		...
		matrModel.PushMatrix(); {
			...
			matrModel.Translate(0.0, 0.0, -taille - (longMembre / 2.0));
