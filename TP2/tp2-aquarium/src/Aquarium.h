...
public:
        ...
        : locillumination(-1)
    {
        ...
        for ( unsigned int i = 0 ; i < sizeof(hauteur)/sizeof(hauteur[0]) ; ++i )
        {
            ...
            // partie 2: modifs ici ...
            (*p).couleurSel = glm::vec3(1.0 - i / float(sizeof(hauteur)), 0.0, 0.5);
    ...
    void selectionnerTheiere()
    {
        ...
        // Pour obtenir la couleur du pixel sous le curseur 0 à 255
        GLubyte color[3];
        glReadPixels(posX, posY, 1, 1, GL_RGB, GL_UNSIGNED_BYTE, color);
        for (std::vector<Theiere*>::iterator it = theieres.begin(); it != theieres.end(); it++)
        {
            float colorHex = (*it)->couleurSel[0] * 255;
            if (colorHex <= color[0] + 0.5 && colorHex > color[0] - 0.5) {
