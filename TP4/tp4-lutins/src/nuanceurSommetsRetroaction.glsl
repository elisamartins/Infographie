...
void main( void )
{
    if ( tempsDeVieRestant <= 0.0 )
    {
        ...
        tempsDeVieRestantMod = mix(0.0, tempsDeVieMax, myrandom(seed++)); // à modifier pour une valeur aléatoire entre 0 et tempsDeVieMax secondes
        ...
        ColorMod = vec4( mix(COULMIN, COULMAX, myrandom(seed++)),
                         ...
                         mix(COULMIN, COULMAX, myrandom(seed++)),
                         1);// composantes de couleur aléatoires
    }
    else
    {
        ...
        if (VertexMod.z < hauteurPlancher) {
            ...
            vitesseMod.z =  -vitesseMod.z/2;
        }

        // appliquer la gravité
        vitesseMod.z -= dt * gravite;
    }
}
